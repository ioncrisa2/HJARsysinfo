...
class PembandingQueryBuilder
{
    ...
    public function withPeruntukan(array $peruntukanSlugs): self
    {
        if (!empty($peruntukanSlugs)) {
            // Filter using relationship and slugs
            $this->query->whereHas('peruntukanRef', function ($q) use ($peruntukanSlugs) {
                $q->whereIn('slug', $peruntukanSlugs);
            });
        }

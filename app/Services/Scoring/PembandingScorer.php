...
class PembandingScorer
{
    ...
    {
        $zoningScore = $this->groupService->getZoningScoreBySlug(
            $input->peruntukanRef?->slug,
            $data->peruntukanRef?->slug
        );
    ...
    {
        $inputArea = ($input->luas_tanah ?? 0) + ($input->luas_bangunan ?? 0);
        $dataArea = ($data->luas_tanah ?? 0) + ($data->luas_bangunan ?? 0);
    ...
    protected function scoreLegal(Pembanding $input, Pembanding $data): float
    {
        ...
        // Get slug from relationship, fallback to 'lainnya'
        $inputSlug = strtolower($input->dokumenTanah?->slug ?? 'lainnya');
        $dataSlug = strtolower($data->dokumenTanah?->slug ?? 'lainnya');

        $inputRank = $legalRanks[$inputSlug] ?? 1;
        $dataRank = $legalRanks[$dataSlug] ?? 1;
    ...
    {
        // Compare slugs from relationships
        $inputSlug = $input->posisiTanah?->slug;
        $dataSlug = $data->posisiTanah?->slug;
    ...
    protected function scoreCondition(Pembanding $input, Pembanding $data): int
    {
        ...
        // Get slug from relationship, fallback to 'sawah'
        $inputSlug = strtolower($input->kondisiTanah?->slug ?? 'sawah');
        $dataSlug = strtolower($data->kondisiTanah?->slug ?? 'sawah');

        $inputRank = $conditionRanks[$inputSlug] ?? 1;
        $dataRank = $conditionRanks[$dataSlug] ?? 1;

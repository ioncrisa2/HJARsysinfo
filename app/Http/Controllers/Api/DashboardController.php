...
#[Group('Dashboard', 'Statistik, ringkasan data, dan peta sebaran untuk dashboard aplikasi.', weight: 3)]
class DashboardController extends Controller
{
    use ApiResponse;

    #[Endpoint(
        title: 'Lihat data dashboard',
    ...
    {
        $user = $request->user();
        $isDataContributor = (bool) $user?->hasRole('data_contributor');

        $can = AppAccess::capabilityMap($user, [
            'viewData' => 'view_any_data::pembanding',
            'createData' => 'create_data::pembanding',
            'view_data' => 'view_any_data::pembanding',
            'create_data' => 'create_data::pembanding',
            'view_any_data' => 'view_any_data::pembanding',
        ]);
        $canWidgets = AppAccess::capabilityMap($user, AppAccess::widgetPermissionMap());

        $pendingDeleteRequests = $user && $user->can('view_moderation')
        ...
        $data = [
            'dashboard_variant' => $isDataContributor ? 'data_contributor' : 'default',
            'map_points' => $mapPoints,
            'stats' => $stats,
            'jenis_listing_options' => $jenisListingOptions,
            'can' => $can,
            'can_widgets' => $canWidgets,
            'delete_request_alert' => $deleteRequestAlert,
        ];
        ...
        $listingRatioMonthly = ['labels' => [], 'series' => [], 'month_totals' => []];
        ...
        if ($canWidgets['topContributorTable']) {
        ...
        $dataFreshness = ['basis' => 'Berdasarkan tanggal_data', 'total' => 0, 'with_date' => 0, 'missing_date' => 0, 'buckets' => []];
        if ($canWidgets['dataFreshnessWidget']) {
            ...
            $freshnessTotal = (int) ($stats['total'] ?? Pembanding::count());
        ...
        $objectTypeCounts = ['total_records' => 0, 'rows' => []];
        if ($canWidgets['objectTypeCountTable']) {
        ...
        $data['recent_data'] = $recentData;
        $data['monthly_data'] = $monthlyData;
        $data['listing_ratio_monthly'] = $listingRatioMonthly;
        $data['top_contributors'] = $topContributors;
        $data['data_freshness'] = $dataFreshness;
        $data['top_area_activity'] = $topAreaActivity;
        $data['object_type_counts'] = $objectTypeCounts;

    ...
    private function searchPembanding(string $keyword): Collection
    {
        return Pembanding::query()
            ...
            ->map(fn (Pembanding $record): array => $this->resultRow(
                ...
                url: "/admin/pembanding/{$record->id}",
                details: [
                    'ID' => "#{$record->id}",
    ...
    private function searchModeration(string $keyword): Collection
    {
        $requests = PembandingDeleteRequest::query()
            ...
            ->map(fn (PembandingDeleteRequest $request): array => $this->resultRow(
                ...
                url: "/admin/moderation?tab=requests&search={$request->id}",
                details: [
        ...
        $trashed = Pembanding::onlyTrashed()
            ...
            ->map(fn (Pembanding $record): array => $this->resultRow(
                ...
                url: "/admin/moderation?tab=trashed&search={$record->id}",
                details: [
            ...
            $results = $results->merge(
                $modelClass::query()
                    ...
                    ->map(fn ($record): array => $this->resultRow(
                        ...
                        resourceName: $label,
                        title: $record->name,
                        url: "/admin/master-data/{$slug}?search=" . urlencode((string) $record->name),
                        details: [
                            'Slug' => $record->slug,
            ...
            $results = $results->merge(
                $modelClass::query()
                    ...
                    ->map(fn ($record): array => $this->resultRow(
                        ...
                        resourceName: $label,
                        title: $record->name,
                        url: "/admin/geo/{$slug}?search=" . urlencode((string) $record->id),
                        details: [

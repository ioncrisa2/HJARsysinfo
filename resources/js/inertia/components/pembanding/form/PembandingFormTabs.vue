...
// Field keys yang belong ke masing-masing tab, untuk badge error
const tabErrorKeys = {
    umum: [
        "jenis_listing_id", "jenis_objek_id",
        "nama_pemberi_informasi", "nomer_telepon_pemberi_informasi",
        "status_pemberi_informasi_id", "tanggal_data",
    ],
    lokasi: [
        "alamat_data", "province_id", "regency_id",
        "district_id", "village_id", "latitude", "longitude",
    ],
    properti: [
        "image", "luas_tanah", "luas_bangunan", "lebar_depan",
        "lebar_jalan", "tahun_bangun", "rasio_tapak",
        "bentuk_tanah_id", "posisi_tanah_id", "kondisi_tanah_id",
        "topografi_id", "dokumen_tanah_id", "peruntukan_id", "harga",
    ],
    catatan: ["catatan"],
};
...
const tabs = [
    { value: "umum",     label: "Informasi Umum", icon: "pi-info-circle" },
...
<template>
    <div class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        <Tabs v-model:value="activeTabModel">
            <TabList class="border-b border-slate-100 bg-slate-50/60 px-2 sm:px-4">
                <Tab v-for="tab in tabs" :key="tab.value" :value="tab.value">
                    <div class="relative flex items-center gap-2 px-1 py-1">
                        <i :class="`pi ${tab.icon}`" style="font-size: 12px" />
                        <span class="hidden text-sm sm:inline">{{ tab.label }}</span>
                        <!-- Error badge -->
                        <span
                            v-if="tabErrorCount[tab.value] > 0"
                            class="absolute -right-2 -top-1 flex h-4 w-4 items-center justify-center rounded-full bg-red-500 text-[9px] font-bold text-white shadow-sm"
                        >
            ...
            <TabPanels>
                <TabPanel value="umum">
                    <PembandingGeneralTab
                        :form="form"
                        :options="options"
                        @next="activeTabModel = 'lokasi'"
                    />
                </TabPanel>

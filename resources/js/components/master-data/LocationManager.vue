...
const loadData = async () => {
    ...
    try {
        ...
        if (currentLevel.value === "province") {
            endpoint = `/app/master-data/locations/provinces`;
        } else if (currentLevel.value === "regency") {
            endpoint = `/app/master-data/locations/regencies?province_id=${currentParent.value.id}${sq}`;
        } else if (currentLevel.value === "district") {
            endpoint = `/app/master-data/locations/districts?regency_id=${currentParent.value.id}${sq}`;
        } else if (currentLevel.value === "village") {
            endpoint = `/app/master-data/locations/villages?district_id=${currentParent.value.id}${sq}`;
        }
    ...
    try {
        ...
        if (currentLevel.value === "province") {
            endpoint = dialogMode.value === "add"
                ? `/app/master-data/locations/provinces`
                : `/app/master-data/locations/provinces/${form.value.original_id}`;
            if (dialogMode.value === "add") body.id = form.value.id;
        } else if (currentLevel.value === "regency") {
            endpoint = dialogMode.value === "add"
                ? `/app/master-data/locations/regencies`
                : `/app/master-data/locations/regencies/${form.value.original_id}`;
            body.province_id = currentParent.value.id;
        } else if (currentLevel.value === "district") {
            endpoint = dialogMode.value === "add"
                ? `/app/master-data/locations/districts`
                : `/app/master-data/locations/districts/${form.value.original_id}`;
            body.regency_id = currentParent.value.id;
        } else if (currentLevel.value === "village") {
            endpoint = dialogMode.value === "add"
                ? `/app/master-data/locations/villages`
                : `/app/master-data/locations/villages/${form.value.original_id}`;
...
const deleteItem = async () => {
    ...
    try {
        let endpoint = "";
        if (currentLevel.value === "province") endpoint = `/app/master-data/locations/provinces/${itemToDelete.value.id}`;
        else if (currentLevel.value === "regency") endpoint = `/app/master-data/locations/regencies/${itemToDelete.value.id}`;
        else if (currentLevel.value === "district") endpoint = `/app/master-data/locations/districts/${itemToDelete.value.id}`;
        else if (currentLevel.value === "village") endpoint = `/app/master-data/locations/villages/${itemToDelete.value.id}`;

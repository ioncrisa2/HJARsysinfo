...
const loadItems = async ({ preserveSelection = true } = {}) => {
    ...
    try {
        const payload = await apiRequest(`/app/master-data/dictionaries/${props.type}`);
        items.value = payload ?? [];
...
const submit = async () => {
    ...
    try {
        ...
        if (isEditing.value) {
            await apiRequest(`/app/master-data/dictionaries/${props.type}/${selectedId.value}`, {
                method: "PUT",
                body: payload,
            });
            emit("success", "Data diperbarui");
        } else {
            const created = await apiRequest(`/app/master-data/dictionaries/${props.type}`, {
                method: "POST",
                body: payload,
            });
...
const confirmDelete = async () => {
    ...
    try {
        errorMessage.value = "";
        await apiRequest(`/app/master-data/dictionaries/${props.type}/${id}`, { method: "DELETE" });
...
const saveOrder = async () => {
    ...
    try {
        errorMessage.value = "";
        await apiRequest(`/app/master-data/dictionaries/${props.type}/reorder`, {
            method: "POST",

...
useDebouncedWatch(search, (value) => {
    router.get("/app/moderation", { search: value, tab: props.tab }, { preserveState: true, replace: true });
}, { delay: 300 });

const switchTab = (t) => {
    search.value = "";
    router.get("/app/moderation", { tab: t }, { preserveState: false });
};
...
const approveReq = (id) => {
    confirm.require({
        ...
        accept: () => {
            router.post(`/app/moderation/approve/${id}`, {}, { preserveScroll: true });
        }
    });
};
...
const confirmReject = () => {
    rejectForm.post(`/app/moderation/reject/${activeRequestId.value}`, {
        preserveScroll: true,
        onSuccess: () => {
...
const restoreData = (id) => {
    confirm.require({
        ...
        icon: 'pi pi-refresh',
        accept: () => {
            router.post(`/app/moderation/restore/${id}`, {}, { preserveScroll: true });
        }
    });
};
...
const forceDeleteData = (id) => {
    confirm.require({
        ...
        acceptClass: 'p-button-danger',
        accept: () => {
            router.delete(`/app/moderation/force-delete/${id}`, { preserveScroll: true });
        }
    });
};
...
<template>
    <AppLayout title="Moderation Desk">
                        ...
                        <tr v-for="res in trashedPaginator?.data" :key="res.id" class="align-top transition-colors hover:bg-slate-50/70">
                            ...
                            <td class="px-6 py-5">
                                <div class="flex items-center gap-2">
                                    ...
                                    <span class="min-w-0 truncate text-xs font-bold text-slate-700" :title="res.deleted_by?.name || 'Tidak diketahui'">
                                        {{ res.deleted_by?.name || 'Tidak diketahui' }}
                                    </span>
                                </div>
                            </td>
                            <td class="px-6 py-5">

...
const resourceUrl = (resource) => `/app/geo/${resource}`;
...
const loadRegencies = async (provinceId) => {
    ...
    try {
        ...
        regencyOptions.value = await apiRequest(`/app/geo/lookups/regencies?${query.toString()}`);
    } finally {
...
const loadDistricts = async (regencyId) => {
    ...
    try {
        ...
        districtOptions.value = await apiRequest(`/app/geo/lookups/districts?${query.toString()}`);
    } finally {

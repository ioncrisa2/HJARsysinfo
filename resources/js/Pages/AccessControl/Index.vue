...
const submitRole = () => {
    ...
    if (isEditingRole.value) {
        roleForm.put(`/app/access-control/roles/${editingRoleId.value}`, {
            preserveScroll: true,
    ...
    roleForm.post("/app/access-control/roles", {
        preserveScroll: true,
        onSuccess: closeRoleDialog,
    });
};
...
const deleteRole = (role) => {
    ...
    confirm.require({
        ...
        accept: () => router.delete(`/app/access-control/roles/${role.id}`, { preserveScroll: true }),
    });
};
...
const submitPermission = () => {
    ...
    permissionForm.post("/app/access-control/permissions", {
        preserveScroll: true,
        onSuccess: () => {
...
const deletePermission = (permission) => {
    ...
    confirm.require({
        ...
        icon: "pi pi-exclamation-triangle",
        acceptClass: "p-button-danger",
        accept: () => router.delete(`/app/access-control/permissions/${permission.id}`, { preserveScroll: true }),
    });
};
...
<template>
    <AppLayout title="Access Control">
        <div class="mb-6 space-y-4">
            <Link href="/app/users" class="inline-flex items-center gap-1.5 text-sm text-slate-500 hover:text-slate-800">

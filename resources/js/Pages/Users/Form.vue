...
const submit = () => {
    if (isEditing) {
        form.put(`/app/users/${props.user.id}`);
    } else {
        form.post("/app/users");
    }
};
...
<template>
    <AppLayout :title="isEditing ? 'Edit User' : 'Create User'">

        <div class="mb-4">
            <Link href="/app/users" class="inline-flex items-center gap-1.5 text-sm text-slate-500 hover:text-slate-800 transition">
